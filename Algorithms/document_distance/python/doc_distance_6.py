...
# - Removed insertion sort, replaced with merge sort
...
class Document():
    ...
    def __init__(self, filename):
        ...
        self.readFile()
        self.countFrequency()
        self.ct_list = self.mergeSort(self.ct_list)
    def readFile(self):
    ...
    def calculateDotProduct(doc1, doc2):
        ...
        while (i < len(list1) and j < len(list2)):
            if (list1[i][0] == list2[j][0]):
                ...
                j+=1
        return sum
    def mergeSort(self, list): # needs a param since it's not in place sort
        # merge sort algorithm
        size = len(list)
        if size == 1: # 1 element array; no sorting needed
            return list
        mid = size/2
    ...
    def merge(self, left, right):
        ...
        while (i < len(left) and j < len(right)):
            if (left[i] < right[j]): # left item is smaller
...
def main():
    if (len(sys.argv) != 3):
        print "Usage: python doc_distance_6.py textfile1 textfile2"
    else:
...
# time to finish sample info - 1 seconds